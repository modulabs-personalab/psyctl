...
> 이 프로젝트는 현재 개발 중이며 제한된 기능만 지원합니다. 안정적인 기능은 릴리스 노트를 확인하세요.

[모두의연구소](https://modulabs.co.kr/) [Persona Lab](https://modulabs.co.kr/labs/337) 프로젝트입니다.

LLM이 특정 성격을 나타내도록 조향(steering)하는 도구입니다. 모델과 성격 지정만으로 자동으로 데이터셋을 생성하고 스티어링 벡터를 추출할 수 있습니다.
...
- **[스티어링 데이터셋 생성](./docs/DATASET.BUILD.STEER.md)** - 벡터 추출용 스티어링 데이터셋 생성
- **[스티어링 벡터 추출](./docs/EXTRACT.STEERING.md)** - mean_diff 또는 BiPO 방법으로 스티어링 벡터 추출
- **[스티어링 실험](./docs/STEERING.md)** - CAA(Contrastive Activation Addition)를 사용한 스티어링 벡터 적용

### 추가 자료

- **[설정](./docs/CONFIGURATION.md)** - 환경 변수 및 성능 튜닝
- **[OpenRouter 연동](./docs/OPENROUTER.md)** - 로컬 GPU 대신 클라우드 API 사용
- **[커뮤니티 데이터셋](./docs/COMMUNITY.DATASETS.md)** - 사전 구축된 데이터셋 및 레지스트리
...
> **참고**: `transformers` 패키지가 `torch`를 의존성으로 가지고 있어 `uv sync` 실행 시 CPU 버전이 자동 설치됩니다. GPU 사용을 위해서는 위의 CUDA 설치 명령을 다시 실행해야 합니다.
...
# 2. HuggingFace Hub에 데이터셋 업로드 (선택)
...
| 명령어 | 설명 | 문서 |
|--------|------|------|
| `dataset.build.steer` | 스티어링 데이터셋 생성 | [가이드](./docs/DATASET.BUILD.STEER.md) |
| `dataset.upload` | HuggingFace에 데이터셋 업로드 | [가이드](./docs/DATASET.BUILD.STEER.md#uploading-to-huggingface-hub) |
| `extract.steering` | 스티어링 벡터 추출 | [가이드](./docs/EXTRACT.STEERING.md) |
| `steering` | 생성에 스티어링 적용 | [가이드](./docs/STEERING.md) |
| `benchmark inventory` | 심리 검사 인벤토리로 테스트 (logit 기반) | 아래 참조 |
| `benchmark llm-as-judge` | LLM 판별자로 테스트 (상황 기반 질문) | 아래 참조 |
| `inventory.list` | 사용 가능한 인벤토리 목록 | 아래 참조 |

**벤치마크 방법:**
- **Inventory**: 표준화된 심리 검사 인벤토리(예: IPIP-NEO)를 logit 기반 점수로 사용. 더 객관적이고 재현 가능.
- **LLM as Judge**: 상황 기반 질문을 생성하고 LLM이 응답을 평가. 더 유연하고 맥락 인식.
  - API 기반 판별자(OpenAI, OpenRouter)의 경우 환경 변수 설정:
    - `OPENAI_API_KEY` (OpenAI 모델용)
    - `OPENROUTER_API_KEY` (OpenRouter 모델용)
  - 로컬 모델의 경우 `local-default` (대상 모델 재사용) 또는 `benchmark_config.json`에 커스텀 모델 경로 설정
  - 커스텀 API 서버의 경우 `benchmark_config.json`을 편집하여 서버 구성 추가

### 지원 인벤토리

| 인벤토리 | 영역 | 라이선스 | 비고 |
|----------|------|----------|------|
| IPIP-NEO-300/120 | Big Five | Public Domain | 전체 및 단축형 |
| NPI-40 | 나르시시즘 | 연구용 무료 | 강제 선택형 |
| PNI-52 | 병적 나르시시즘 | CC-BY-SA | Likert 1-6 |
| NARQ-18 | 찬양 및 경쟁 | CC-BY-NC | 2개 하위 척도 |
| MACH-IV | 마키아벨리즘 | Public Domain | Likert 1-5 |
| LSRP-26 | 사이코패시 | Public Domain | 1차 및 2차 |
| PPI-56 | 사이코패시 | 연구용 무료 | 단축형 |
...
# 5. LLM 판별자로 성격 변화 측정
# 참고: API 기반 판별자의 경우 환경 변수 설정 필요:
#   export OPENAI_API_KEY="your-key"        # OpenAI 모델
#   export OPENROUTER_API_KEY="your-key"    # OpenRouter 모델
# 또는 benchmark_config.json에 커스텀 API 서버 설정
...
Colab에서 바로 열어 실행할 수 있습니다 -- 로컬 설정 불필요:
...
> **참고**: 노트북 실행에는 HuggingFace 토큰이 필요합니다 (Colab 보안 비밀에 `HF_TOKEN`으로 설정). GPU 런타임(T4 이상) 권장.

---

## 기여하기

기여를 환영합니다! [기여 가이드](./docs/CONTRIBUTING.md)에서 다음을 확인하세요:
- 개발 환경 설정
- 코드 스타일 및 표준
- 테스트 가이드라인
- Pull Request 프로세스
