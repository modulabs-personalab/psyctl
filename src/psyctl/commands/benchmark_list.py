...
def list_inventories(inventory: str | None):
    ...
    available_inventories = get_available_inventories()

    if inventory:
        # Show detailed info for specific inventory
        try:
            inv = create_inventory(inventory)
            ...
            console.print(f"Questions per Trait: {info['questions_per_trait']}\n")
